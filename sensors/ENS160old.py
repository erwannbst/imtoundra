...
class ENS160:
    ...
    Follows the specifications defined in the official ENS160 data sheet: https://www.sciosense.com/products/environmental-sensors/ens160-digital-multi-gas-sensor/
    ...
    def CO2(self) -> int:
        ...
        print("Raw eCO2 data:", bs)
        return self._translate_pair(bs[0], bs[1])
    ...
    def _translate_pair(self, high: bytearray, low: bytearray) -> int:
        ...
        print(low)
        value = (high[0] << 8) + low
        # value = (int.from_bytes(high, 'big') << 8) + low
        if value >= 0x8000:
            value = -((65535 - value) + 1)
            # value = -(0x10000 - value)
        return value
